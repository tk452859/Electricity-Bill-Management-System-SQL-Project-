...
CREATE TABLE `accounts` (
  ...
  PRIMARY KEY (`acc_id`),
  KEY `cust_id` (`cust_id`),
  CONSTRAINT `accounts_ibfk_1` FOREIGN KEY (`cust_id`) REFERENCES `customer` (`cust_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
INSERT INTO `accounts` VALUES (401,111,'11345','Abhay'),(403,112,'12455','Vishnu'),(405,113,'98754','Anant'),(407,211,'34557','Vijay'),(409,212,'24567','Deekshith'),(411,311,'76314','Farhaan'),(413,312,'54327','Ajay'),(415,313,'74123','Nikhil'),(417,411,'43567','Preetham'),(419,412,'98764','Sridhar'),(421,345,'43561','Rohanjit'),(423,325,'78654','Ayushman'),(425,347,'67452','Anwesh'),(427,367,'91294','Devash'),(429,324,'89123','Tushar');
unlock tables;
...
CREATE TABLE `billing` (
  ...
  KEY `acc_id` (`acc_id`),
  KEY `cust_id` (`cust_id`),
  CONSTRAINT `billing_ibfk_1` FOREIGN KEY (`acc_id`) REFERENCES `accounts` (`acc_id`),
...
Customers with bills greater than ₹5000*/

SELECT * 
FROM billing
WHERE total_amount > 5000;
/*Sort bills by total amount in descending order*/
SELECT *
FROM billing
...
FROM billing;
/*Count number of customers billed*/
SELECT COUNT(DISTINCT cust_id) AS Total_Customers
FROM billing;
/*Grouping*/
/*Total revenue collected per unit price*/
SELECT 
    amount_per_unit, 
    SUM(total_amount) AS Revenue
FROM billing
GROUP BY amount_per_unit;
...
FROM billing
GROUP BY cust_id;
/*month’s highest bill*/
SELECT 
    meter_number, 
    MAX(total_amount) AS Highest_Bill
FROM billing
group by meter_number;
/*Average monthly units consumed per customer*/
SELECT 
    cust_id, 
    AVG(monthly_units) AS Avg_Units
FROM billing
GROUP BY cust_id;
/*Revenue generated for each unit price*/
SELECT 
    amount_per_unit, 
    SUM(total_amount) AS Revenue
FROM billing
...
/*Customers who consumed more than 500 units in a month*/
SELECT *
FROM billing
WHERE monthly_units > 500;
/*Top 5 Customers by revenue*/
SELECT 
    cust_id, 
    SUM(total_amount) AS TotalPaid
FROM billing
GROUP BY cust_id
...
/*Find customers with above-average bills*/
SELECT c.cust_name, b.total_amount
FROM customer c
...
SELECT 
    i.reading_date,
    SUM(b.monthly_units) AS total_units
FROM invoice i
JOIN billing b 
    ON i.meter_number = b.meter_number
GROUP BY i.reading_date
ORDER BY total_units DESC;
/*Seasonal Consumption*/
SELECT  
    CASE  
        WHEN MONTH(i.reading_date) BETWEEN 4 AND 9 THEN 'Summer'  
        ELSE 'Winter'  
    END AS season,  
    SUM(b.monthly_units) AS total_units_consumed,  
    AVG(b.monthly_units) AS avg_units_per_customer  
FROM billing b  
JOIN invoice i  
    ON b.meter_number = i.meter_number  
GROUP BY season;
